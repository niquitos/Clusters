...
public static class DbscanSpansService
{
    ...
    public static void Clusterize(EventModel[] events)
    {
        ...
        var neighborsBuffer = new int[events.Length];

        for (int i = 0; i < events.Length; i++)
        {
            if (!assignedEvents[i])
            {
                Span<int> neighbors = GetNeighbors(i, events, assignedEvents, neighborsBuffer);

                if (neighbors.Length < density)
                {
                    continue;
                }
                else
                {
                    ...
                    ExpandCluster(events, neighbors, clusterId, assignedEvents, neighborsBuffer);
        ...
        int[] neighborsBuffer)
    {
        foreach (var eventIndex in neighbors)
        {
            ...
            {
                Span<int> newNeighbors = GetNeighbors(eventIndex, events, assignedEvents, neighborsBuffer);

                if (newNeighbors.Length >= density)
                {
                    AssignCluster(eventIndex, newNeighbors, clusterId, events, assignedEvents);
                    ExpandCluster(events, newNeighbors, clusterId, assignedEvents, neighborsBuffer);
                }
            }
        }
    }

    private static Span<int> GetNeighbors(int eventIndex,
        EventModel[] events,
        BitArray assignedEvents,
        int[] neighborsBuffer)
    {
        ...
        for (int i = 0; i < events.Length; i++)
        {
            ...
            {
                neighborsBuffer[neighborsCount++] = i;
            }
        }

        return neighborsBuffer.AsSpan(0, neighborsCount);
    }
