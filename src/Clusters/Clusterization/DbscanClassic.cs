...
public static class DbscanArray
{
    private const double epsilon1 = 0.7;
    private const double epsilon2 = 0.6;
    private const double epsilon3 = 0.85;
    ...
    private static void AssignCluster(EventModel @event, EventModel[] neighbors, int clusterId)
    {
        @event.ClusterId = clusterId;
    ...
    private static void ExpandCluster(EventModel[] events, EventModel[] neighbors, int clusterId)
    {
        for (int i = 0; i < neighbors.Length; i++)
        {
            var newNeighbors = GetNeighbors(neighbors[i], events);

            if (newNeighbors.Length != 0)
            {
                AssignCluster(neighbors[i], newNeighbors, clusterId);
                ExpandCluster(events, newNeighbors, clusterId);
            }
        }
    }

    private static EventModel[] GetNeighbors(EventModel @event, EventModel[] events)
    {
        return [.. events
